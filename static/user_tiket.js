          ...
          data.bookings.forEach((booking, index) => {
              ...
              card.innerHTML = `
                  <div class="flex justify-between items-center">
                      <div>
                          <p class="font-semibold text-gray-700">#${index + 1} - ${booking.fullname}</p>
                          <p class="text-sm text-gray-500">Event ID: ${booking.event_id}</p>
                          <p class="text-sm text-gray-500">Metode: ${booking.payment_method || '-'}</p>
                          <p class="text-sm text-gray-500">Status Pembayaran: 
                              <span class="font-medium ${booking.payment_status === 'PAID' ? 'text-green-600' : 'text-red-600'}">
